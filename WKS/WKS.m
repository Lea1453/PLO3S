...
resLInfSum = sum(sum(resLInf,1));
resLNaNSum = sum(sum(resLNaN,1));

resAmInfSum = sum(sum(resAmInf,1));
resAmNaNSum = sum(sum(resAmNaN,1));
...
[sizeL1, sizeL2] = size(L);
[sizeResL1, sizeResL2] = size(resLNaN);

printf("size %d %d size %d %d \n", sizeL1, sizeL2, sizeResL1, sizeResL2);

...
for(i in 1:(n%/%2))
{
	...
	close(f)
	print(headTX)
	sizeX <- as.numeric(strsplit(headTX, " ")[[1]][2])
	sizeY <- as.numeric(strsplit(headTX, " ")[[1]][1])

	tX <- read.table(args[i],skip=1)
	#tY <- read.table(args[2])
	#print(tX)
	mX <- matrix(tX$V63,nrow = sizeX,ncol = sizeY)#<- as.matrix(tX)	
	#mY <- as.matrix(tY)

	#names_obj = read.table(args[3])$V1

	#rgb.palette <- colorRampPalette(c("black","yellow","orange","red"))
	...
	image.plot(t(mX[nrow(mX):1,]), xlab="", ylab="", axes=FALSE, main="Carte de convexite")

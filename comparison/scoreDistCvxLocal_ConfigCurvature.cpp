...
void scoreDistCvx(char **filenamesCvxIn, char **filenamesWKS, char *filenameCvxOut, int sizeac)
 {
  ...
  /*calculate the CPU time */
   ...
   float delta_time_REAL, delta_time_REAL_glob;
  ...
  /*init variable des stats et histogram images*/
   ...
   // PERMET DE CHANGER LA TAILLE DU PATCH (PAS TRES PROPRE):
       ...
       for(i = 0; i < nbImg; i++)
	 {
	   ...
	   for(j = 0; j <= i; j++)
	       ...
	       for(k=0; k < maxSize; k++)
		 {
	       ...
	       //loadFile_mapCvx_mView_allView_WKS(&(filenamesCvxIn[i*NB_MVIEW]), &(acHist[0]), &(size_curv[0]), sizeXY[0], &size_patch, NB_MVIEW);//AJOUT POUR WKS
	       //SUPPRIMER LES 5 LIGNES POUR WKS
	       loadFile_mapCvx_mView_allView(&(filenamesCvxIn[i*NB_MVIEW]), &(acTmp[0]), &(size_curv[0]), sizeXY[0], &size_patch, NB_MVIEW);
	       //AJOUT (PAS TRES PROPRE) POUR CHANGER LA TAILLE DE PATCH
	       size_patch = SIZE_PATCH;
	       ...
	       std::cout << "End of loading descriptor " << i*NB_MVIEW << "\n";
	       ...
	       for(d1 = 0; d1 < DIV_FINDMIN; d1++)
		 {
		   ...
		   //loadFile_mapCvx_mView_allView_WKS(&(filenamesCvxIn[j*NB_MVIEW]), &(acHist[1]), &(size_curv[1]), sizeXY[1], &size_patch, NB_MVIEW); //AJOUT POUR WKS
		   //SUPPRIMER LES 5 LIGNES POUR WKS
		   ...
		   std::cout << "End of loading descriptor " << j*NB_MVIEW << "\n";
		   ...
		   for(d2 = 0; d2 < DIV_FINDMIN; d2++)
		       ...
		       for(l = 0; l < numGrpMult_min; l++)
			 {
		       ...
		       //std::cout << "C5 : " << getCurrentValue() << "\n";
		     } //d2
		   //  std::cout << "B3 : " << getCurrentValue() << "\n";
		   delete[] acHist[1];
		   //std::cout << "B4 : " << getCurrentValue() << "\n";
		 }//d1
	       ...
	       for(k=0; k < maxSize; k++)
		 {	  
		   if(coordMin1_jvsi[k] == -2 || k != coordMin2_jvsi[coordMin1_jvsi[k]] )
		     {
   ...
   for(k = 0; k < nbImg*nbImg; k++)
     {
       matDist[k] = matDist_hist[k]; // * (1000.0/(matNbPatchSelec[k]+1)); // + matDist_wks[k];

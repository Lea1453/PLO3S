...
void scoreDistCvx(char **filenamesCvxIn, char **filenamesWKS, char *filenameCvxOut, int sizeac)
 {
  ...
  /*calculate the CPU time */
   ...
   struct timeval start_time_loadFile, end_time_loadFile;
   float delta_time_REAL, delta_time_REAL_glob, delta_time_loadFile;
       ...
       for(i = 0; i < nbImg; i++)
	 {
	   ...
	   for(j = 0; j <= i; j++)
	       ...
	       for(k=0; k < maxSize; k++)
		 {
	       ...
	       gettimeofday(&start_time_loadFile, NULL);                                   
	       loadBinaryFile_mapCvx_mView_allView_WKS(&(filenamesCvxIn[i*NB_MVIEW]), &(acHist[0]), &(size_curv[0]), sizeXY[0], &size_patch, NB_MVIEW);

	       gettimeofday(&end_time_loadFile, NULL);                              
               delta_time_loadFile = ((end_time_loadFile.tv_sec  - start_time_loadFile.tv_sec) * 1000000u + end_time_loadFile.tv_usec - start_time_loadFile.tv_usec) / 1.e6;                                                                     
               std::cout << "time in REAL sec for loading 1 file : " << delta_time_loadFile << "\n";      
			 
		   loadBinaryFile_mapCvx_mView_allView_WKS(&(filenamesCvxIn[j*NB_MVIEW]), &(acHist[1]), &(size_curv[1]), sizeXY[1], &size_patch, NB_MVIEW);

		   int sizeCurvMin;
		   
		   if(size_curv[0] < size_curv[1])
		     sizeCurvMin = size_curv[0];
		   else
		     sizeCurvMin = size_curv[1];


		   matDist_hist[j*nbImg+i] = 0.0;
		   
		   for(d2 = 0; d2 < sizeCurvMin; d2++)
		     {

		       matDist_hist[j*nbImg+i] += fabs(acHist[0][d2] - acHist[1][d2]);
		     }
		   ...
		   printf("Numéro image : %d %d  score courbure : %f, taille(i,j) : (%d,%d)", i,j, matDist_hist[j*nbImg+i],size_gen[i], size_gen[j]);
